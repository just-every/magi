...
describe('Test Provider', () => {
    ...
    describe('Callback-based API', () => {
        it('should work with callback-based createResponse method', async () => {
            const events: EnsembleStreamEvent[] = [];
            const errors: unknown[] = [];
            
            testProviderConfig.fixedResponse = 'Callback test';
            testProviderConfig.streamingDelay = 10;
        ...
        it('should handle errors in callback API', async () => {
            ...
            testProviderConfig.shouldError = true;
            testProviderConfig.errorMessage = 'Callback error test';
            ...
            // Should have error event but no error callback (errors go to events)
            const errorEvent = events.find(e => e.type === 'error');
            expect(errorEvent).toBeDefined();
            expect((errorEvent as any).error).toBe('Callback error test');
        });
