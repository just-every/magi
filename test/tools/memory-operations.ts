...
export default async function memoryOperationsTest(options: MemoryOperationsOptions = {}): Promise<ToolResult> {
  ...
  // Function availability is guaranteed by the TypeScript declarations
  ...
  try {
    ...
    const saveResult = await save_memory('long', JSON.stringify(testMemoryValue));
    ...
    const memoryId: string = idMatch && idMatch[1] ? idMatch[1] : '';
    ...
    const findResultBeforeDelete = await find_memory([testMemoryKey]);
    if (verbose) console.log('Find memory result (before delete):', findResultBeforeDelete);

    if (!findResultBeforeDelete || !(findResultBeforeDelete.includes(testMemoryValue.data) && findResultBeforeDelete.includes(testMemoryValue.timestamp.toString()))) {
      return {
        success: false,
    ...
    const deleteResult = await delete_memory('long', parseInt(memoryId));
    ...
    const findResultAfterDelete = await find_memory([testMemoryKey]);
    ...
    if (findResultAfterDelete && findResultAfterDelete.includes(testMemoryValue.data) && findResultAfterDelete.includes(testMemoryValue.timestamp.toString())) {
      return {
        success: false,
