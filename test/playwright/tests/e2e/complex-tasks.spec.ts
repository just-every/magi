...
test.describe('Complex Task E2E', () => {
  test('should handle a multi-step task (write file, execute command)', async ({ page }) => {
    ...
    const taskInput = page.locator('textarea[placeholder*="Enter your task"]'); // Adjust selector
    ...
    await expect(outputContainer).toContainText(expectedOutput, { timeout: 90000 });
    await expect(outputContainer).toContainText(`Successfully saved to ${scriptFileName}`, { timeout: 10000 }); // Check file write confirmation
    await expect(outputContainer).toContainText(`Successfully deleted ${scriptFileName}`, { timeout: 10000 }); // Check deletion confirmation
  ...
  test('should handle a code modification task (create, modify, read)', async ({ page }) => {
    ...
    const modificationInstruction = 'In the function greet, change "Hello, world!" to "Hello, modified world!"';
    ...
    await expect(outputContainer).toContainText(modifiedContent, { timeout: 90000 });
    await expect(outputContainer).toContainText(`Successfully saved to ${codeFileName}`, { timeout: 10000 }); // Initial write
    ...
    await expect(outputContainer).toContainText(`Successfully deleted ${codeFileName}`, { timeout: 10000 }); // Deletion
  });

  test('should handle task combining browser read, file write, and command execution', async ({ page }) => {
    ...
    await expect(outputContainer).toContainText(`Navigated to ${targetUrl}`, { timeout: 90000 }); // Browser nav
    await expect(outputContainer).toContainText(expectedHeading, { timeout: 10000 }); // Browser read result / File read result / Echo command output
    await expect(outputContainer).toContainText(`Successfully saved to ${tempFileName}`, { timeout: 10000 }); // File write
    await expect(outputContainer).toContainText(`The heading read from the website was: ${expectedHeading}`, { timeout: 10000 }); // Command execution output
    await expect(outputContainer).toContainText(`Successfully deleted ${tempFileName}`, { timeout: 10000 }); // Deletion
  });

  test('should handle task involving search and reporting results', async ({ page }) => {
    // Navigate to the Controller UI
    await page.goto(CONTROLLER_URL);
    await expect(page.locator('body')).toBeVisible({ timeout: 10000 });
    ...
    await expect(outputContainer).toContainText('Found matches for', { timeout: 90000 }); // General confirmation
    await expect(outputContainer).toContainText('magi/src/magi_agents/common_agents/browser_agent.ts', { timeout: 10000 }); // Example file expected to have TODOs
