...
test.describe('Basic Controller Interaction E2E', () => {
  test('should load the controller UI and allow starting a simple task', async ({ page }) => {
    ...
    await expect(outputContainer).toContainText('package.json', { timeout: 15000 });
    await expect(outputContainer).toContainText('README.md', { timeout: 15000 });

    // Optional: Add checks for task status indicators (e.g., "Completed")
  });

  test('should display a message or prevent submission for an empty task', async ({ page }) => {
    // Navigate to the Controller UI
    await page.goto(CONTROLLER_URL);
    ...
    await expect(errorMessage).toContainText('Task cannot be empty', { timeout: 5000 }); // Adjust expected text
  ...
  test('should handle tasks that result in an error', async ({ page }) => {
    ...
    const outputContainer = page.locator('.output-container, .error-container'); // Adjust selector
    await expect(outputContainer).toBeVisible({ timeout: 30000 });

    // Verify an error message is displayed
    // This depends on how errors are presented in the UI
    await expect(outputContainer).toContainText('Error', { ignoreCase: true, timeout: 15000 });
    await expect(outputContainer).toContainText('foobar123.txt', { timeout: 15000 }); // Check if the error mentions the file
    // TODO: Add more specific checks for error formatting or codes if available
  });

  test('should handle file operations (create, read, delete)', async ({ page }) => {
    ...
    await expect(outputContainer).toContainText(`Successfully saved to ${testFileName}`, { timeout: 30000 });
    ...
    await expect(outputContainer).toContainText(testFileContent, { timeout: 30000 });
    ...
    await expect(outputContainer).toContainText(`Successfully deleted ${testFileName}`, { timeout: 30000 }); // Adjust expected text
  });
  ...
  test('should execute a shell command via a task', async ({ page }) => {
    ...
    await expect(outputContainer).toContainText('Hello from execute_command test!', { timeout: 30000 });
  ...
  test('should display status updates during and after task execution', async ({ page }) => {
    // Navigate to the Controller UI
    await page.goto(CONTROLLER_URL);
    await expect(page.locator('body')).toBeVisible({ timeout: 10000 });
    ...
    await expect(statusIndicator).toContainText(/Running|Processing/i, { timeout: 5000 }); // Adjust expected text/regex
    ...
    await expect(outputContainer).toContainText('package.json', { timeout: 15000 }); // Check for some expected output

    // Check for a "Completed" or "Finished" status
    await expect(statusIndicator).toContainText(/Completed|Finished|Done/i, { timeout: 5000 }); // Adjust expected text/regex
  });
