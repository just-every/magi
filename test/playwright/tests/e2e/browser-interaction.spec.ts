...
test.describe('Browser Interaction E2E', () => {
  test('should open a URL in the browser via a task', async ({ page }) => {
    ...
    await expect(outputContainer).toContainText(`Navigated to ${targetUrl}`, { timeout: 45000 }); // Adjust expected text and timeout
  ...
  test('should interact with form elements (type and click) via tasks', async ({ page }) => {
    // Navigate to the Controller UI
    await page.goto(CONTROLLER_URL);
    await expect(page.locator('body')).toBeVisible({ timeout: 10000 });
    ...
    await expect(outputContainer).toContainText("Clicked element button[type=\"submit\"]", { timeout: 60000 }); // Adjust expected text and timeout
  ...
  test('should read content from a web page via a task', async ({ page }) => {
    ...
    await expect(outputContainer).toContainText('Example Domain', { timeout: 45000 }); // Check for the expected h1 text
  });

  test('should use vision to interact with elements based on visual context', async ({ page }) => {
    // Navigate to the Controller UI
    await page.goto(CONTROLLER_URL);
    await expect(page.locator('body')).toBeVisible({ timeout: 10000 });
    ...
    await expect(outputContainer).toContainText(/Clicked element/i, { timeout: 60000 }); // General check for click confirmation
  ...
  test('should handle errors when interacting with non-existent browser elements', async ({ page }) => {
    // Navigate to the Controller UI
    await page.goto(CONTROLLER_URL);
    await expect(page.locator('body')).toBeVisible({ timeout: 10000 });
    ...
    const taskInput = page.locator('textarea[placeholder*="Enter your task"]'); // Adjust selector
    const submitButton = page.locator('button:has-text("Run")'); // Adjust selector
    const outputContainer = page.locator('.output-container, .error-container'); // Adjust selector to include error display
    ...
    await expect(outputContainer).toContainText(/Element not found|Could not find element|selector.*#non-existent-button/i, { timeout: 45000 }); // Adjust regex/text
  });

  test('should take a screenshot of a web page via a task', async ({ page }) => {
    // Navigate to the Controller UI
    await page.goto(CONTROLLER_URL);
    await expect(page.locator('body')).toBeVisible({ timeout: 10000 });
    ...
    await expect(outputContainer).toContainText(/Screenshot taken|Screenshot saved/i, { timeout: 45000 }); // Adjust regex/text
  ...
  test('should close a browser tab via a task', async ({ page }) => {
    ...
    await expect(outputContainer).toContainText(/Closed tab|Tab closed/i, { timeout: 60000 }); // Adjust regex/text
