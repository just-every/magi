...
test.describe('Agent Tool Calls', () => {
  test('should invoke tools and process results', async ({ configureTestProvider }) => {
    ...
    const agent = new Agent({
      ...
      instructions: 'You are a test agent that can search for information',
      model: 'test-standard',
    ...
    const handlers = {
      onToolCall: async (toolCall) => {
        ...
        expect(JSON.parse(toolCall.function.arguments).query).toBe('test query');
      },
    ...
    expect(toolResults[0].result).toContain('Search results for: test query');
    
    // Verify the final response incorporates the search results
    expect(response).toContain('answer is 42');
  });
  
  test('should handle multiple tool calls', async ({ configureTestProvider }) => {
    ...
    const runnerPromise = (async () => {
      try {
          ...
          [],
          {
            onToolCall: async (toolCall) => {
              ...
              if (toolCalls.length === 1) {
                ...
                testProviderConfig.toolArguments = { text: 'transform this text' };
              }
              
              // After the second tool call, disable tool calls for the final response
              if (toolCalls.length === 2) {
                testProviderConfig.simulateToolCall = false;
                testProviderConfig.fixedResponse = 'Final response after using multiple tools';
              }
            },
            onEvent: (event) => {
              events.push(event);
        ...
        expect(response).toBe('Final response after using multiple tools');
      } catch (error) {
    ...
    expect(JSON.parse(toolCalls[0].function.arguments).query).toBe('first query');
    
    // Verify the second tool call was text_transform
    expect(toolCalls[1].function.name).toBe('text_transform');
    expect(JSON.parse(toolCalls[1].function.arguments).text).toBe('transform this text');
  });
  
  test('should respect maxToolCalls limit', async ({ configureTestProvider }) => {
      ...
      [],
      {
        ...
        onEvent: (event) => {
          ...
          if (toolCallCount >= agent.maxToolCalls) {
            testProviderConfig.simulateToolCall = false;
            testProviderConfig.fixedResponse = 'Final forced response without tools';
          }
        }
      }
    );
    ...
    expect(eventTypes.filter(t => t === 'tool_start')).toHaveLength(agent.maxToolCalls);
    expect(eventTypes.filter(t => t === 'message_complete')).toHaveLength(toolCallCount + 1);
  });
});