...
test.describe('Quota Manager', () => {
    test('should track quota usage correctly', async () => {
        ...
        const result = quotaManager.trackUsage(
            'google',
        ...
        expect(summary.google.dailyUsed).toBeGreaterThan(
            initialQuota.dailyLimit
        );
    });
    ...
    test('should track credit usage correctly', async () => {
        ...
        expect(summary.openai.creditBalance).toBe(
    ...
    test('should reset daily quotas on day change', async () => {
        ...
        expect(quotaAfterReset.lastResetDate.getDate()).toBe(
            new Date().getDate()
        ); // Updated to today
    });
});
