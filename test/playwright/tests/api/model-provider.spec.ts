...
import { getModelProvider, getModelFromClass, getProviderFromModel } from '../../../../magi/src/model_providers/model_provider.js';
import { MODEL_CLASSES, ModelClassID } from '../../../../magi/src/model_providers/model_data.js';
...
test.describe('Model Provider API', () => {
  test('should get the correct provider for each model', async () => {
    ...
    expect(getProviderFromModel('claude-3-7-sonnet-latest')).toBe('anthropic');
  ...
  test('should get models for each class', async () => {
    ...
    for (const [className, modelClass] of Object.entries(MODEL_CLASSES)) {
      ...
      const hasTestModel = modelClass.models.some(model => model.startsWith('test-'));
  ...
  test('should select models with quota available', async ({ configureTestProvider }) => {
    ...
    for (const modelClass of Object.keys(MODEL_CLASSES) as ModelClassID[]) {
      ...
      const belongsToClass = MODEL_CLASSES[modelClass].models.includes(model);
