...
test.describe('Runner Fallback Mechanism', () => {
  test('should fallback to next model on error', async ({ configureTestProvider }) => {
      ...
      try {
        ...
        for await (const event of stream) {
          ...
          if (event.type === 'error') {
            ...
            testProviderConfig.fixedResponse = 'Response from fallback model';
          }
        }
      } catch (error) {
  ...
  test('should fallback from rate-limited model to paid model', async ({ configureTestProvider }) => {
      ...
      try {
        ...
        for await (const event of stream) {
          ...
          if (event.type === 'error') {
            ...
            testProviderConfig.fixedResponse = 'Response from paid Gemini model';
          }
        }
      } catch (error) {
        console.error('Runner error:', error);
  ...
  test('should try all available models before giving up', async ({ configureTestProvider }) => {
    ...
    const runnerPromise = (async () => {
      try {
        const stream = Runner.runStreamed(agent, 'Test all models input');
