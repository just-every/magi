...
import { TestProvider, testProviderConfig } from '../../../../magi/src/model_providers/test_provider.js';

test.describe('Test Provider', () => {
  test('should return a basic response', async ({ configureTestProvider }) => {
    ...
    const stream = provider.createResponseStream('test-standard', [{ role: 'user', content: 'Hello' }]);
  ...
  test('should simulate an error', async ({ configureTestProvider }) => {
    ...
    // Create a response stream
    const stream = provider.createResponseStream('test-error', [{ role: 'user', content: 'Trigger an error' }]);
  ...
  test('should simulate a rate limit error', async ({ configureTestProvider }) => {
    ...
    // Create a response stream
    const stream = provider.createResponseStream('test-rate-limit', [{ role: 'user', content: 'Trigger a rate limit' }]);
  ...
  test('should simulate a tool call', async ({ configureTestProvider }) => {
    ...
    const stream = provider.createResponseStream('test-standard', [{ role: 'user', content: 'Search for something' }], mockAgent as any);
    ...
    expect(JSON.parse(toolCallEvent.tool_calls[0].function.arguments)).toEqual({ query: 'test query' });
  ...
  test('should generate thinking content', async ({ configureTestProvider }) => {
    // Configure the test provider
    configureTestProvider({
    ...
    // Create a response stream
    const stream = provider.createResponseStream('test-reasoning', [{ role: 'user', content: 'Complex question' }]);
