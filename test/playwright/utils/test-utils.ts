...
import fs from 'fs';
import { testProviderConfig, resetTestProviderConfig } from '../../../magi/src/model_providers/test_provider.js';
...
export const test = base.extend({
  ...
  createTempFile: async ({}, use) => {
    ...
    const createFile = (content: string, extension = '.txt'): string => {
      const filename = path.join(process.cwd(), `temp_${Date.now()}${extension}`);
  ...
  waitForOutput: async ({}, use) => {
    ...
    const waitForFileOutput = async (filepath: string, expectedContent: string | RegExp, timeout = 10000): Promise<boolean> => {
      ...
      while (Date.now() - startTime < timeout) {
        if (fs.existsSync(filepath)) {
          ...
          if (typeof expectedContent === 'string' && content.includes(expectedContent)) {
            return true;
          } else if (expectedContent instanceof RegExp && expectedContent.test(content)) {
            return true;
          }
        }
