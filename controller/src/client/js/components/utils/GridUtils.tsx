...
export const calculateBoxPositions = (
    ...
    const { x: coreX, y: coreY } = gridToScreenCoords(coreGridPos.row, coreGridPos.col, originX, originY, squareWidth, squareHeight);
    ...
    const coreWorkers = coreProcess.agent?.workers ? Array.from(coreProcess.agent.workers.keys()) : [];
    ...
    rowMinus1ItemIds.forEach((itemId, index) => {
        ...
        const { x, y } = gridToScreenCoords(itemGridPos.row, itemGridPos.col, originX, originY, squareWidth, squareHeight);
    ...
    processes.forEach((process, id) => {
        ...
        if (process.agent?.workers && process.agent.workers.size > 0) {
            ...
            if (!parentGridPos) {
                console.error(`Could not find grid position for parent process ${id} when placing workers.`);
                return;
            }
            ...
            workers.forEach((workerId, workerIndex) => {
                ...
                const workerGridRow = parentGridPos.row - 1 - Math.floor(workerIndex / 2);
                ...
                const { x: cellX, y: cellY } = gridToScreenCoords(workerGridRow, workerGridCol, originX, originY, squareWidth, squareHeight);

                // Determine position within the cell (left or right)
                const isLeftWorker = workerGridCol < 1 ? (workerIndex % 2 !== 0) : (workerIndex % 2 === 0);
                ...
                const xOffset = isLeftWorker ? 0 : (squareWidth / 2) + (GRID_PADDING / 4); // Adjusted offset for right worker
...
export const calculateBoundingBox = (positions: Map<string, BoxPosition>): {
    ...
    if (!Number.isFinite(minX + minY + maxX + maxY)) {
        console.warn("Bounding box calculation resulted in non-finite values. Resetting to origin.");
...
export const calculateZoomToFit = (
    ...
    if (contentWidth <= 0 || contentHeight <= 0 || !Number.isFinite(contentWidth + contentHeight)) {
        if (boxPositions.size === 1 && Number.isFinite(contentWidth + contentHeight)) {
            ...
            const zoomFitX = containerSize.width / Math.max(1, contentWidth + 2 * GRID_PADDING);
            const zoomFitY = containerSize.height / Math.max(1, contentHeight + 2 * GRID_PADDING);
            ...
            console.warn("Zoom calculation: Zero area content or non-finite bounds, returning default view.");
