...
export const calculateBoxPositions = (
    ...
    const taskRadius = squareWidth * 2;

    taskIds.forEach((taskId, index) => {
        const angle = (index / taskIds.length) * 2 * Math.PI;
    ...
    processes.forEach((process, id) => {
        ...
        workerIds.forEach((workerId, wIndex) => {
            if (positionsMap.has(workerId)) return;
            const angle = (wIndex / workerIds.length) * 2 * Math.PI;
            const x =
                parentPos.x +
                parentPos.width / 2 +
