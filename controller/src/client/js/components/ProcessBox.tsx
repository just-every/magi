...
import { useSocket, MagiMessage, ToolCallMessage, ToolResultMessage } from '../context/SocketContext';
...
const ProcessBox: React.FC<ProcessBoxProps> = ({ 
  ...
  const getStatusClass = () => {
    switch (status) {
      case 'running': return 'status-running bg-light';
      case 'completed': return 'status-completed bg-success';
      case 'failed': return 'status-failed bg-warning';
      case 'ending':
      case 'terminated': return 'status-terminated bg-danger';
      default: return 'status-running bg-light';
    }
  };
  ...
  const renderMessages = () => {
    ...
    const messageMap = new Map<string, MagiMessage>();
                ...
                <div className="message-bubble tool-bubble">
                  ...
                  {toolCallMsg.command && (
                    <div className="tool-call-command">
                      <pre className="command-line"><span className="prompt">$</span> {toolCallMsg.command}</pre>
                    </div>
                  )}
  ...
  return (
    <div 
      className={`process-box card border-0 shadow ${focused ? 'focused' : ''} ${isSubAgent ? 'sub-agent' : ''}`}
      onClick={handleBoxClick}
    >
      <div 
        className="card-header d-flex justify-content-between align-items-center border-0 pe-2"
      ...
      >
        <div className="d-flex align-items-center">
          {isSubAgent && (
            <span 
              className="sub-agent-indicator me-2" 
              title="Sub-agent"
          ...
          {hasChildProcesses && (
            <span 
              className="parent-agent-indicator ms-2" 
        ...
        <div className="d-flex align-items-center gap-2">
          ...
          <span 
            className={`process-status status-label btn-sm ${getStatusClass()}`}
            style={{ color: status === 'running' ? colors.textColor : '' }}
          >
            {status}
          </span>
          {status !== 'ending' && status !== 'terminated' && (
            <button 
              className="process-terminate btn btn-sm btn-outline-danger"
