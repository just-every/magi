...
import {CommandMessage, CommunicationManager} from '../managers/communication_manager';
...
function handleIncomingMessage(chatId: number, text: string): void {
	...
	try {
		...
		const success = communicationManager.sendMessage(coreProcessId, JSON.stringify(commandMessage));
