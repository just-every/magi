 ...
 */
export async function initColorManager(): Promise<void> {
    usedColors = await loadUsedColors();
}
 ...
 */
export async function saveUsedColors(
    colors: Array<RGBColor> = usedColors
): Promise<void> {
    await saveData('USED_COLORS', JSON.stringify(colors));
}
 ...
 */
export async function loadUsedColors(): Promise<Array<RGBColor>> {
    const colorsJson = await loadData('USED_COLORS');
    if (!colorsJson) {
        return [];
    }
 ...
 */
export function generateProcessColors(isCore: boolean): {
    rgb: string;
    bgColor: string;
    textColor: string;
} {
