/**
 * Tool definitions for the Design Agent
 */

import { createToolFunction, type ToolFunction } from '@just-every/ensemble';
import { web_search, createNumberedGrid, selectBestFromGrid, type ImageSource } from '../design-search.js';
import { generate_image_raw } from '../design-image.js';
// Constants removed - unused in this file

/**
 * Get all tools for the design agent
 */
export function getDesignAgentTools(): ToolFunction[] {
    ...
    return [
        ...
        /*
        // Get design specifications tool
        createToolFunction(
            (args: { assetType: string }) => {
                const assetType = args.assetType as DESIGN_ASSET_TYPES;
                const reference = DESIGN_ASSET_REFERENCE[assetType];
                const guide = DESIGN_ASSET_GUIDE[assetType];

                if (!reference) {
                    return {
                        error: `Unknown asset type: ${assetType}`,
                        availableTypes: Object.keys(DESIGN_ASSET_REFERENCE),
                    };
                }
            ...
            'Get design specifications and guidelines for a specific asset type',
            {
                assetType: {
                    type: 'string',
                    enum: Object.keys(DESIGN_ASSET_REFERENCE),
                    description: 'Type of design asset to get specifications for',
                },
            },
            'Returns specifications and guidelines for the asset type',
            'get_design_specifications'
        ),
