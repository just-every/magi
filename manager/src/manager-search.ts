...
import {
    ...
    MANAGER_SEARCH_DESCRIPTIONS,
    MANAGER_SEARCH_ENGINES,
    type MANAGER_ASSET_TYPES,
    type ManagerAssetAspect,
...
export const MANAGER_ASSETS_DIR = DEFAULT_ASSETS_DIR;
...
async function takeScreenshot(
    ...
    try {
        ...
        const filePath = path.join(MANAGER_ASSETS_DIR, 'screenshots', filename);
...
function parseSearchResults(searchResult: string, limit: number): ManagerSearchResult[] {
    ...
    try {
        ...
        let currentResult: Partial<ManagerSearchResult> | null = null;
        ...
        for (const line of lines) {
            ...
            if (urlMatch && urlMatch[0]) {
                if (currentResult && currentResult.url) {
                    results.push(currentResult as ManagerSearchResult);
                }
                currentResult = {
                    url: urlMatch[0],
 ...
 */
export async function smart_manager_raw(
    context: string,
    searchConfigs: {
        engine: ManagerSearchEngine;
        query: string;
    ...
    console.log(`[smart_manager_raw] Running ${searchConfigs.length} search configs`);
    ...
    const searchPromises = searchConfigs.map(async (config) => {
        try {
            const result = await manager_search(
                config.engine,
                `${background}\n\n${config.query}`,
                config.limit || 9
            );
    ...
    console.log(`[smart_manager_raw] Found ${allDesigns.length} total designs from all searches`);
    ...
    while (currentCandidates.length > finalLimit && round <= 3) {
        ...
        for (let i = 0; i < groups.length; i++) {
            ...
            const gridDataUrl = await createNumberedGrid(
                imageSources,
                gridName,
                type ? MANAGER_ASSET_REFERENCE[type].spec.aspect : 'square'
            );
