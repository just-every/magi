...
const DEFAULT_OUTPUT_DIR = process.env.MANAGER_OUTPUT_DIR || path.join(process.cwd(), '.output');
...
const communicationManager = {
    send: (data: any) => {
        console.log(`[ManagerImage] ${data.type}:`, {
...
/**
 * High level manager search with iterative vision-based ranking to generate an enhanced image
 * This now uses the agent-based approach with MECH
 */
export async function manager_image(
    ...
    const { simpleManagerImage } = await import('./simple-manager.js');
    
    return await simpleManagerImage(type, prompt, with_inspiration, brand_assets);
}
...
export function getManagerImageTools(): ExtendedToolFunction[] {
    return [
        createExtendedToolFunction(
            manager_image,
            'An intelligent process that searches the web for reference images, then runs multiple passes to generate aesthetically pleasing managers. Can be used to manage logos, websites, and other visual content.',
            {
                type: {
                    description: 'What type of manager to create',
                    type: 'string',
                    enum: ['color_pallet', 'primary_logo', 'homepage_mockup'],
                    default: 'primary_logo',
                },
                prompt: 'A text description of the desired manager',
                with_inspiration: {
                    description:
                        'The manager process will look at reference images from the web to help inspire the manager. This will take longer, but the results are usually significantly better.',
                    type: 'boolean',
                    default: 'true',
                },
                brand_assets: {
