...
export function onNativeMessage(message: NativeMessage): void {
  ...
  if (!message || typeof message !== 'object') {
    console.error('[messaging] Invalid message received from native host:', message);
    return;
  }
  ...
  processCommand(command, tabId || '', params || {})
    ...
    .catch((error: Error) => {
      console.error(`[messaging] Error processing command '${command}':`, error);
      sendErrorResponse(requestId, `Failed to process command '${command}': ${error.message}`, error.stack);
    });
}

// Send a response back to the native host
export function sendResponse(requestId: number, response: ResponseMessage): void {
  if (!nativePort) {
    console.error('[messaging] Cannot send response: No connection to native host.');
    return;
  }
  ...
  try {
    ...
    console.error('[messaging] Failed to send response to native host:', error);
  }
}
...
export function sendErrorResponse(
  ...
  console.error(`[messaging] Sending error response for request ${requestId}:`, errorMessage, details || '');
...
export function connectNativeHost(): void {
  try {
    console.log(`[messaging] Connecting to native messaging host: ${NATIVE_HOST_NAME}`);
    ...
    console.error('[messaging] Failed to connect to native messaging host:', error);
    setNativePort(null);
  }
}
...
export function onDisconnected(): void {
  ...
  if (error) {
    console.error('[messaging] Native host disconnected with error:', error);
  } else {
...
export function checkAndReconnectNativeHost(): void {
  if (!nativePort) {
    console.log('[messaging] No native host connection, attempting to connect...');
    connectNativeHost();
  }
}