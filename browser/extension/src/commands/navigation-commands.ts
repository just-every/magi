...
export async function navigateHandler(
  ...
  console.log(`[navigation-commands] Navigating tab ${tabId} to URL: ${params.url}`);
  ...
  try {
    ...
    console.error(`[navigation-commands] Navigation failed for ${tabId}:`, error);
    return {
      status: 'error',
...
export async function getUrlHandler(
  ...
  try {
    ...
    console.error(`[navigation-commands] Failed to get URL for ${tabId}:`, error);
    return {
      status: 'error',
