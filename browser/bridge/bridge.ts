            ...
            arg =>
                typeof arg === 'string'
                    ...
                    : util.inspect(arg, {
                      ...
                      }) // Use colors only if stderr is a TTY
        )
        .join(' ');
...
process.on('unhandledRejection', (reason, promise) => {
    ...
    if (reason instanceof Error) {
        ...
        logError(
            'Reason:',
            util.inspect(reason, { depth: 3, colors: process.stderr.isTTY })
        );
    }
...
nativeInput.on('end', () => {
    ...
    try {
        ...
        logError(
            `WebSocket server status: ${serverListening ? 'Listening (or closing)' : 'Closed'}`
        );
        logError(
            `Number of active WS clients: ${activeWsClients?.size ?? 'N/A'}`
        );
    } catch (e) {
        logError('Error logging state during stdin end:', e);
    }
    logError('Closing WebSocket connections now...');
    if (wss && !wss_closed) {
        wss.close();
    } else {
        logError('WebSocket server already closed or not initialized.');
    }
...
function processNativeMessageQueue() {
    while (true) {
        ...
        if (currentNativeMessageLength === null) {
            logError('Internal state error...');
            break;
        }

        const combinedBuffer = Buffer.concat(nativeMessageQueue);
        if (combinedBuffer.length < currentNativeMessageLength) {
            nativeMessageQueue = [combinedBuffer];
            break;
        }

        const messageBuffer = combinedBuffer.slice(
            0,
            currentNativeMessageLength
        );
        const remainingBuffer = combinedBuffer.slice(
            currentNativeMessageLength
        );
        ...
        logError(
            `Raw message received (Length: ${messageLength}): ${messageJson}`
        );

        try {
            const response = JSON.parse(messageJson) as ExtensionResponse;
            if (response.requestId === undefined || !response.status) {
                logError(`Invalid response format...`, response);
                continue;
            }
            logError(
                `EXT->BRIDGE] Native ID ${response.requestId} Status: ${response.status}`,
                response.result || response.error || ''
            );
            const pendingRequest = pendingNativeRequests.get(
                response.requestId
            );
            if (pendingRequest) {
                ...
                if (response.status === 'error') {
                    logError(`Relaying error from extension...`);
                }
                ...
                if (response.requestId === 0) {
                    logError(`Received message with requestId 0...`);
                } else {
                    logError(
                        `Received native response for unknown/timed-out request ID: ${response.requestId}`
                    );
                }
            }
        } catch (error) {
            ...
            logError(
                `Raw message JSON content that failed parsing:`,
                messageJson
            );
        }
...
try {
    ...
    wss.on('connection', (ws: WebSocket) => {
        ...
        ws.on('message', async (message: Buffer) => {
            ...
            try {
                ...
                if (!command.wsRequestId || !command.command) {
                    throw new Error(
                        'Invalid command format: wsRequestId and command are required.'
                    );
                }
            } catch (error) {
                logError(
                ...
                sendWebSocketResponse(ws, {
                    wsRequestId: 'unknown',
                    status: 'error',
                    error: 'Invalid message format received by bridge.',
                });
                return;
            }
            ...
            const timeoutId = setTimeout(() => {
                if (pendingNativeRequests.has(nativeRequestId)) {
                    ...
                    logError(errorMsg);
                    sendWebSocketResponse(pending.wsClient, {
                        wsRequestId: pending.wsRequestId,
                        status: 'error',
                        error: 'Request to browser extension timed out.',
                        details: `Native Request ID: ${nativeRequestId}`,
                    });
                }
            }, NATIVE_MSG_TIMEOUT);
            pendingNativeRequests.set(nativeRequestId, {
                wsClient: ws,
                wsRequestId: command.wsRequestId,
                timeoutId: timeoutId,
            });
            try {
                const nativeMessage = {
                ...
                if (nativeOutput.writable) {
                    ...
                    logError(
                        `BRIDGE->EXT] Native ID ${nativeRequestId} (from WS ID ${command.wsRequestId}): ${command.command}`,
                        command.params || {}
                    );
                } else {
                    logError(
                        'Error: Native output stream is not writable when trying to send message.'
                    );
                    ...
                    sendWebSocketResponse(ws, {
                        wsRequestId: command.wsRequestId,
                        status: 'error',
                        error: 'Bridge failed to send command to extension: Native output stream closed.',
                        details: `Native Request ID: ${nativeRequestId}`,
                    });
                }
            } catch (error) {
                ...
                sendWebSocketResponse(ws, {
                    wsRequestId: command.wsRequestId,
                    status: 'error',
                    error: `Bridge failed to send command to extension: ${error instanceof Error ? error.message : String(error)}`,
                    details: `Native Request ID: ${nativeRequestId}`,
                });
            }
        });
        ws.on('close', (code, reason) => {
            ...
            pendingNativeRequests.forEach((pending, nativeId) => {
                if (pending.wsClient === ws) {
                    clearTimeout(pending.timeoutId);
                    requestsToRemove.push(nativeId);
                    logError(
                        `Clearing pending native request ${nativeId} due to WS client disconnect (WS ID: ${pending.wsRequestId}).`
                    );
                }
            });
        ...
        ws.on('error', error => {
            ...
            pendingNativeRequests.forEach((pending, nativeId) => {
                if (pending.wsClient === ws) {
                    ...
                    requestsToRemove.push(nativeId);
                    logError(
                        `Clearing pending native request ${nativeId} due to WS client error (WS ID: ${pending.wsRequestId}).`
                    );
                }
            });
            requestsToRemove.forEach(id => pendingNativeRequests.delete(id));
    ...
    wss.on('error', error => {
        ...
        try {
            if (!nativeInput.destroyed) nativeInput.destroy();
        } catch (e) {
            logError('Error destroying native input on WSS error:', e);
        }
        process.exit(1);
    });
    wss.on('close', () => {
        logError('WebSocket server closed.');
        wss_closed = true;
        try {
            if (!nativeInput.destroyed) nativeInput.destroy();
        } catch (e) {
            logError('Error destroying native input on WS close:', e);
        }
    });
    logError(`WebSocket server listening on ws://localhost:${WEBSOCKET_PORT}`);
} catch (e) {
    logError('Failed to initialize WebSocket server:', e);
    process.exit(1);
}
    ...
    wsHeartbeatInterval = setInterval(() => {
        activeWsClients.forEach((ws: WebSocket) => {
            if ((ws as any).isAlive === false) {
                logError('WebSocket client unresponsive...');
                return ws.terminate();
            }
...
function sendWebSocketResponse(
    ...
    if (wsClient.readyState === WebSocket.OPEN) {
        try {
            ...
            logError(
                `BRIDGE->WS] Sent response for WS ID ${response.wsRequestId}, Status: ${response.status}`
            );
        } catch (error) {
            logError(`Failed to send WebSocket response...`, error);
            wsClient.terminate();
        }
    } else {
        logError(
            `Cannot send WebSocket response... client state is ${wsClient.readyState}`
        );
        const requestsToRemove: number[] = [];
        pendingNativeRequests.forEach((pending, nativeId) => {
            if (pending.wsClient === wsClient) {
                clearTimeout(pending.timeoutId);
                requestsToRemove.push(nativeId);
                logError(
                    `Clearing pending native request ${nativeId} as WS client state is not OPEN (WS ID: ${pending.wsRequestId}).`
                );
            }
        });
        requestsToRemove.forEach(id => pendingNativeRequests.delete(id));
...
function gracefulShutdown(signal: string) {
    ...
    if (wss && !wss_closed) {
        ...
        logError(
            'WebSocket server already closed or not initialized, proceeding with shutdown.'
        );
        shutdownNativeConnection();
    }
    setTimeout(() => {
...
function shutdownNativeConnection() {
    try {
        if (!nativeInput.destroyed) {
            logError('Closing native input stream...');
            nativeInput.destroy();
        }
    } catch (e) {
        logError('Error destroying native input during shutdown:', e);
    } finally {
