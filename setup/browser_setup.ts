...
export async function setupBrowserExtension(): Promise<boolean> {
    ...
    try {
        ...
        if (!extensionId) {
            ...
            console.log(
                '\x1b[33m%s\x1b[0m',
                '4. After installation, copy the extension ID (a string like "abcdefghijklmnopqrstuvwxyzabcdef")'
            );
            console.log(
                '\x1b[33m%s\x1b[0m',
                '   The ID appears under the extension name in the extensions list\n'
            );
            ...
            if (!extensionId || extensionId.length < 32) {
                console.log(
                    '\x1b[31m%s\x1b[0m',
                    'Invalid extension ID. Please try again with a valid ID.'
                );
                return false;
            }
        ...
        if (!nativeMessagingHostDir) {
            console.log(
                '\x1b[31m%s\x1b[0m',
                'Unsupported platform for automatic native messaging host installation.'
            );
            console.log(
                '\x1b[33m%s\x1b[0m',
                'Please manually install the native messaging host manifest:'
            );
            console.log(
                '\x1b[33m%s\x1b[0m',
                '1. Edit browser/bridge/com.withmagi.magi_native_host.json'
            );
            console.log(
                '\x1b[33m%s\x1b[0m',
                '2. Replace YOUR_EXTENSION_ID_HERE with your extension ID'
            );
            console.log(
                '\x1b[33m%s\x1b[0m',
                '3. Update the "path" value to the full absolute path to bridge_runner.sh'
            );
            console.log(
                '\x1b[33m%s\x1b[0m',
                '4. Place this file in the appropriate location for your OS'
            );
            console.log(
                '\x1b[33m%s\x1b[0m',
            ...
            console.log(
                '\x1b[33m%s\x1b[0m',
                '   - Windows: Registry HKEY_CURRENT_USER\\Software\\Google\\Chrome\\NativeMessagingHosts\\'
            );
            return false;
        }
        ...
        const sourceManifestPath = path.resolve(
            process.cwd(),
            'browser/bridge/com.withmagi.magi_native_host.json'
        );
        const targetManifestPath = path.join(
            nativeMessagingHostDir,
            'com.withmagi.magi_native_host.json'
        );
        const bridgeRunnerPath = path.resolve(
            process.cwd(),
            'browser/bridge/bridge_runner.sh'
        );
...
exit $EXIT_CODE`;
        ...
        console.log(
            '\x1b[32m%s\x1b[0m',
            'Successfully installed the native messaging host manifest.'
        );
        console.log(
            '\x1b[32m%s\x1b[0m',
            `Manifest location: ${targetManifestPath}`
        );
        console.log(
            '\x1b[32m%s\x1b[0m',
            `Bridge runner path: ${bridgeRunnerPath}`
        );
