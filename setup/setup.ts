...
function getRecentDirectories(baseDir: string, limit: number = 10): string[] {
  try {
    ...
    const dirs = entries
      ...
      .filter(item => item !== null) as { path: string, name: string, mtime: number }[];

    // Sort by modification time (most recent first)
    const sortedDirs = dirs.sort((a, b) => b.mtime - a.mtime);

    // Check if magi-system is in the list
    const magiSystemDir = sortedDirs.find(dir => dir.name === 'magi-system');
    ...
    if (!magiSystemPath && !rootDir.endsWith('magi-system')) {
      ...
      if (fs.existsSync(possibleMagiPath) && fs.existsSync(path.join(possibleMagiPath, '.git'))) {
    ...
    console.error(`Error getting recent directories: ${error instanceof Error ? error.message : String(error)}`);
    return [];
  }
}
...
function handleDirectorySelection(): void {
  ...
  console.log('\x1b[32m%s\x1b[0m', 'MAGI will only be able to access the directories you select here.');
  console.log('\x1b[33m%s\x1b[0m', 'Changes will be saved as git branches and only merged with your approval.');

  // Check if PROJECT_REPOSITORIES is already set
  if (envVars['PROJECT_REPOSITORIES'] && envVars['PROJECT_REPOSITORIES'].trim() !== '') {
    ...
    rl.question('\nWould you like to update your directory access settings? (y/n): ', (answer) => {
      if (answer.toLowerCase() !== 'y' && answer.toLowerCase() !== 'yes') {
        console.log('\x1b[32m%s\x1b[0m', `✓ Keeping current directory access settings`);
  ...
  function setupDirectorySelection() {
    ...
    function displayDirectories() {
      ...
      fullDirList.forEach((dir, index) => {
        ...
        const indicator = isSelected ? `\x1b[32m✓\x1b[0m` : `\x1b[31m✗\x1b[0m`;
      ...
      // Display options
      console.log('\nOptions:');
      console.log('  - Enter directory numbers to toggle selection (e.g., "2 3" to toggle #2 and #3)');
    ...
    function handleGitRepository(dirPath: string, callback: (success: boolean) => void) {
      const isGitRepo = fs.existsSync(path.join(dirPath, '.git'));

      if (!isGitRepo) {
        console.log(`\x1b[33m%s\x1b[0m`, `Warning: ${dirPath} is not a Git repository.`);
        rl.question('Would you like to initialize Git in this directory? (y/n): ', (gitAnswer) => {
          if (gitAnswer.toLowerCase() === 'y' || gitAnswer.toLowerCase() === 'yes') {
            try {
              // Initialize Git repository
              console.log(`Initializing Git repository in ${dirPath}...`);
              execSync(`git -C "${dirPath}" init`, { stdio: 'inherit' });
              console.log(`\x1b[32m%s\x1b[0m`, `Git repository initialized in ${dirPath}`);
              callback(true);
            } catch (error) {
              console.log(`\x1b[31m%s\x1b[0m`, `Error initializing Git: ${error instanceof Error ? error.message : String(error)}`);
              callback(false);
            }
          } else {
            console.log(`\x1b[31m%s\x1b[0m`, `Directory cannot be used without Git. Not adding ${dirPath}`);
            callback(false);
          }
        });
      } else {
    ...
    const promptForSelection = () => {
      rl.question('\nSelection (press Enter to accept): ', (input) => {
        ...
        if (input === '' || input.toLowerCase() === 'c') {
          ...
          console.log(`\x1b[32m%s\x1b[0m`, `✓ Selected directories: ${dirList}`);
          promptForMissingKeys();
          return;
          ...
          input.split(/\s+/).forEach(numStr => {
            ...
            if (num >= 1 && num <= fullDirList.length) {
              ...
              if (selectedDirs.has(dir)) {
                ...
                console.log(`\x1b[31m%s\x1b[0m`, `Removed: ${dir}`);
              } else {
                selectedDirs.add(dir);
                console.log(`\x1b[32m%s\x1b[0m`, `Added: ${dir}`);
              }
            } else {
              console.log(`\x1b[33m%s\x1b[0m`, `Warning: Number ${num} is out of range`);
            }
          });
          ...
          try {
            ...
            if (stats.isDirectory()) {
              ...
              if (!fullPath.startsWith(parentDir)) {
                ...
                console.log(`\x1b[33m%s\x1b[0m`, `For security reasons, only directories within ${parentDir} can be accessed`);
                displayDirectories();
                promptForSelection();
                return;
              }
              ...
              handleGitRepository(input, (success) => {
                if (success) {
                  ...
                  if (selectedDirs.has(input)) {
                    ...
                    console.log(`\x1b[31m%s\x1b[0m`, `Removed: ${input}`);
                  } else {
                    selectedDirs.add(input);
                    console.log(`\x1b[32m%s\x1b[0m`, `Added: ${input}`);
                  }
                }
...
function ensureAllEnvVars(): void {
  ...
  if (fs.existsSync(envPath)) {
    ...
    if (missingVars.length === 0) {
      ...
      rl.question('\nWould you like to edit the current variables? (y/n): ', (answer) => {
        if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
          console.log('\x1b[36m%s\x1b[0m', '\nEditing environment variables:');
          console.log('\x1b[90m%s\x1b[0m', 'Press Enter to keep the current value, or enter a new value to change it.\n');
          ...
          console.log('\x1b[32m%s\x1b[0m', 'Keeping current environment variables.');
          promptForMissingKeys();
        }
      });
      return;
...
function promptForMissingKeys(): void {
  ...
  const configPrompts = [
    ...
    },
    {
      ...
      description: 'Telegram Bot API token for remote communication via Telegram. To create a bot, message /newbot to @BotFather to get your API token. IMPORTANT: After setup, you must send a message to your bot first before it can message you.'
    },
    {
      key: 'TELEGRAM_ALLOWED_CHAT_IDS',
      prompt: 'Enter your Telegram chat IDs (comma-separated for multiple IDs, press Enter to skip): ',
      infoUrl: 'https://t.me/userinfobot',
      description: 'Telegram chat IDs allowed to communicate with MAGI. Message @userinfobot on Telegram to get your chat ID. NOTE: You must initiate a conversation with your bot for messages to work.'
    },
    {
      key: 'PROJECT_REPOSITORIES',
  ...
  if (nextPrompt) {
    ...
    if (nextPrompt.customPrompt && nextPrompt.key === 'PROJECT_REPOSITORIES') {
    ...
    if (isEditing && currentValue && nextPrompt.key.includes('API_KEY') && currentValue.length > 8) {
...
function saveEnvFile(): void {
  try {
    ...
    const configKeys = [
      ...
      { key: 'TELEGRAM_BOT_TOKEN', comment: '# Telegram Bot API token for remote communication' },
      { key: 'TELEGRAM_ALLOWED_CHAT_IDS', comment: '# Comma-separated list of allowed Telegram chat IDs' },
      { key: 'PROJECT_REPOSITORIES', comment: '# Directory names (relative to parent dir) that Magi can access, separated by commas' }
    ];
...
function buildDockerImage(): void {
	...
	try {
		execSync('docker build --no-cache -t magi-system:latest -f magi/docker/Dockerfile magi', {
			stdio: 'inherit',
			cwd: rootDir
		});
...
function setupClaude(): void {
	...
	if (!checkDockerInstalled()) {
		...
		rl.question('Do you want to skip Claude setup and continue? (y/n): ', (answer) => {
			if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
...
function setupBrowser(): void {
	...
	console.log('\x1b[90m%s\x1b[0m', 'The Browser Extension allows MAGI to interact with Chrome.');
	rl.question('Do you want to set up the Browser Extension? (y/n): ', (answer) => {
		if (answer.toLowerCase() === 'n' || answer.toLowerCase() === 'no') {
			console.log('\x1b[33m%s\x1b[0m', 'Skipping Browser Extension setup. You can run "npm run setup:browser" later if needed.');
			setupComplete();
			return;
		}
		...
		try {
			...
			console.log('\x1b[33m%s\x1b[0m', 'Follow the prompts to install the Chrome extension when they appear.');

			execSync('npm run setup:browser', {stdio: 'inherit', cwd: rootDir});
			console.log('\x1b[32m%s\x1b[0m', '✓ Browser Extension setup completed successfully');
			setupComplete();
		} catch (error) {
			console.error('\x1b[31m%s\x1b[0m', 'Failed to set up Browser Extension.');
			console.error('Error: ', error instanceof Error ? error.message : String(error));

			rl.question('Do you want to continue without Browser Extension setup? (y/n): ', (answer) => {
				if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
					console.log('\x1b[33m%s\x1b[0m', 'Skipping Browser Extension setup. You can run "npm run setup:browser" later.');
					setupComplete();
				} else {
					console.log('Setup aborted. Please fix the Browser Extension setup issue and try again.');
					process.exit(1);
				}
			});
		}
