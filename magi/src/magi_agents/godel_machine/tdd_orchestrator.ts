...
export class TestRunner {
    ...
    private async detectTestRunner(): Promise<string> {
        try {
            ...
            const packageJson = await this.runCommand(
            ...
            const hasJestConfig = await this.runCommand(
                'find . -maxdepth 1 -name "jest.config.*" -o -name "jest.config.json" | wc -l'
            );
            if (parseInt(hasJestConfig.trim()) > 0) return 'jest';

            const hasMochaConfig = await this.runCommand(
                'find . -maxdepth 1 -name ".mocharc.*" -o -name ".mocharc.json" | wc -l'
            );
            if (parseInt(hasMochaConfig.trim()) > 0) return 'mocha';

            const hasVitestConfig = await this.runCommand(
                'find . -maxdepth 1 -name "vitest.config.*" -o -name "vitest.config.json" | wc -l'
            );
...
`;
        ...
        try {
            ...
            if (
                !planJson ||
                !planJson.features ||
                !Array.isArray(planJson.features)
            ) {
                throw new Error(
                    'Invalid planning result format. Expected JSON with a features array.'
                );
            }

            // Validate and process features
            this.plan.features = planJson.features.map(
                (feature: any, index: number) => {
                    const id = feature.id || index + 1;

                    if (!feature.description) {
                        throw new Error(
                            `Feature ${id} is missing a description`
                        );
                    }

                    if (!feature.test_file_path) {
                        console.warn(
                            `Feature ${id} is missing a test file path, will be determined during test writing`
                        );
                    }

                    if (!feature.implementation_file_path) {
                        console.warn(
                            `Feature ${id} is missing an implementation file path, will be determined during implementation`
                        );
                    }
            ...
            console.log(
                `Planning complete. Identified ${this.plan.features.length} features.`
            );
        } catch (error) {
    ...
    private async gatherProjectContext(): Promise<string> {
        ...
        try {
            ...
            const fileStructure = await this.runShellCommand(
                'find . -type f -name "*.ts" -o -name "*.js" | grep -v "node_modules" | sort'
            );

            // 2. Check the package.json for dependencies and scripts
            let packageJson = '{}';
            try {
                packageJson = await this.runShellCommand(
                    'cat package.json 2>/dev/null || echo "{}"'
                );
            } catch (error) {
            ...
            let tsConfig = '{}';
            try {
                tsConfig = await this.runShellCommand(
                    'cat tsconfig.json 2>/dev/null || echo "{}"'
                );
            } catch (error) {
            ...
            const testFiles = await this.runShellCommand(
                'find . -type f -name "*.test.ts" -o -name "*.spec.ts" -o -name "*Test.ts" | grep -v "node_modules" | sort'
            );
            ...
            try {
                ...
                if (testFileList.length > 0) {
                    for (let i = 0; i < Math.min(3, testFileList.length); i++) {
                        const content = await this.runShellCommand(
                            `cat ${testFileList[i]} 2>/dev/null || echo "// File not found"`
                        );
                        sampleFileContents += `\n--- ${testFileList[i]} ---\n${content}\n`;
                    }
                }
                ...
                const sourceFiles = await this.runShellCommand(
                    'find ./src -type f -name "*.ts" | grep -v ".test.ts" | grep -v ".spec.ts" | head -3'
                );
                const sourceFileList = sourceFiles
                    .trim()
                    .split('\n')
                    .filter(line => line.trim() !== '');
...
`;
    ...
    private async implementFeatures(projectContext: string): Promise<void> {
        ...
        for (const feature of sortedFeatures) {
            console.log(
                `Processing feature ${feature.id}: ${feature.description}`
            );
            ...
            try {
                ...
                const testContent = await this.writeTests(
                    feature,
                    projectContext
                );
                this.featureResults.set(feature.id, {
                    test_content: testContent,
                });

                // 2. Verify RED: Run tests (they should fail)
                feature.status = 'running_tests_red';
                const redResult = await this.testRunner.runTests(
                    feature.test_file_path!
                );
                this.featureResults.get(feature.id)!.test_result_red =
                    redResult;
                ...
                if (redResult.passed) {
                    console.warn(
                        `WARNING: Tests for feature ${feature.id} passed on first run, but we expected them to fail. This might indicate insufficient testing.`
                    );
                }

                // 3. GREEN Phase: Write implementation code to make tests pass
                feature.status = 'writing_code';
                const implementationContent = await this.writeImplementation(
                    feature,
                    testContent,
                    redResult
                );
                ...
                const greenResult = await this.testRunner.runTests(
                    feature.test_file_path!
                );
                this.featureResults.get(feature.id)!.test_result_green =
                    greenResult;
                ...
                if (!greenResult.passed) {
                    ...
                    while (!greenResult.passed && retry < this.maxRetries) {
                        retry++;
                        console.log(
                            `Tests still failing for feature ${feature.id}. Retry ${retry}/${this.maxRetries}...`
                        );

                        // Try to fix the implementation based on test failures
                        const fixedImplementation =
                            await this.fixImplementation(
                                feature,
                                testContent,
                                implementationContent,
                                greenResult
                        ...
                        this.featureResults.get(
                            feature.id
                        )!.implementation_content = fixedImplementation;

                        // Run tests again
                        const retryResult = await this.testRunner.runTests(
                            feature.test_file_path!
                        );
                        this.featureResults.get(feature.id)!.test_result_green =
                            retryResult;
                        ...
                        if (retryResult.passed) {
                            console.log(
                                `Tests passing after retry ${retry} for feature ${feature.id}`
                            );
                            break;
                        }
                    }
                    ...
                    if (
                        ...
                        console.error(
                            `Failed to implement feature ${feature.id} after ${this.maxRetries} retries`
                        );
                        continue;
                    }
                }
                ...
                if (
                    ...
                    this.featureResults.get(feature.id)!.refactored_content =
                        refactoredContent;

                    // Verify refactored code still passes tests
                    const refactorResult = await this.testRunner.runTests(
                        feature.test_file_path!
                    );
                    if (!refactorResult.passed) {
                        // Revert to pre-refactored implementation if tests fail
                        console.warn(
                            `Refactored code for feature ${feature.id} failed tests. Reverting to previous implementation.`
                        );

                        // Write original implementation back
                        await this.testRunner.writeFile(
                            feature.implementation_file_path!,
                            this.featureResults.get(feature.id)!
    ...
    private sortFeaturesByDependencies(): Feature[] {
        ...
        const visit = (featureId: number) => {
            ...
            if (temp.has(featureId)) {
                throw new Error(
                    `Circular dependency detected involving feature ${featureId}`
                );
            }
...
`;
        ...
        if (!feature.test_file_path) {
            ...
            console.warn(
                `No test file path specified for feature ${feature.id}, using default: ${feature.test_file_path}`
            );
        }
        ...
        if (!feature.implementation_file_path) {
            ...
            console.warn(
                `No implementation file path specified for feature ${feature.id}, using default: ${feature.implementation_file_path}`
            );
        }
        ...
        if (codeBlocks && codeBlocks.length > 0) {
            ...
            const match = codeBlocks[0].match(
                /```(?:typescript|javascript|ts|js)?\n([\s\S]*?)```/
            );
            if (match && match[1]) {
                testCode = match[1];
            ...
            if (codeLines.length > 0) {
                ...
                throw new Error(
                    `Could not extract test code for feature ${feature.id}`
                );
            }
        }
     ...
     */
    private async writeImplementation(
        feature: Feature,
        testCode: string,
        ...
        let existingCode = '';
        try {
            existingCode = await this.testRunner.readFile(
                feature.implementation_file_path!
            );
        } catch (error) {
...
`;
        ...
        if (codeBlocks && codeBlocks.length > 0) {
            ...
            if (codeLines.length > 0) {
                ...
                throw new Error(
                    `Could not extract implementation code for feature ${feature.id}`
                );
            }
        }
...
`;
        ...
        if (codeBlocks && codeBlocks.length > 0) {
            ...
            if (codeLines.length > 0) {
                ...
                throw new Error(
                    `Could not extract fixed implementation code for feature ${feature.id}`
                );
            }
        }
...
`;
        ...
        if (
            result.includes('The code is already well-structured') ||
        ...
        if (codeBlocks && codeBlocks.length > 0) {
            ...
            console.log(
                `Could not extract refactored code for feature ${feature.id}, keeping original`
            );
            return implementationCode;
        }
        ...
        console.log(
            `Refactored implementation written for feature ${feature.id}`
        );
    ...
    private generateReport(finalTestResult: TestResult): string {
        ...
        // Summary
        const completedFeatures = this.plan.features.filter(
            f => f.status === 'completed'
        ).length;
        const failedFeatures = this.plan.features.filter(
            f => f.status === 'failed'
        ).length;
