...
export async function taskForceContext(
    agent: Agent,
 ...
 */
export function task_complete(result: string): never {
    // Return type 'never' as it always throws
 ...
 */
export function task_fatal_error(error: string): never {
    // Return type 'never' as it always throws
...
export async function runTask(input: string): Promise<string> {
    ...
    try {
        ...
        console.warn(
            '[TaskRun] runSequential completed without a TaskComplete or TaskFatalError signal.'
        );
        return (
            finalOutput ||
            'Task sequence completed without explicit success/error signal.'
        );
    } catch (error) {
        ...
        if (error instanceof TaskCompleteSignal) {
            console.log(
                `[TaskRun] Task completed successfully: ${error.result}`
            );
            comm.send({
                type: 'process_done',
                output: error.result,
            ...
            console.error(
                '[TaskRun] Unexpected error during task execution:',
                error
            );
            throw error;
        }
