...
export function loadMemory(): Memory {
  try {
    ...
    if (fs.existsSync(memoryPath)) {
      ...
      console.log(`Loaded memory with ${memoryCache.inputs.length} inputs and ${memoryCache.outputs.length} outputs`);
    } else {
...
export function getConversationHistory(): Array<{role: string, content: string}> {
  ...
  const maxItems = Math.max(memoryCache.inputs.length, memoryCache.outputs.length);
