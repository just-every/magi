                ...
                json_schema: {
                    ...
                    properties: {
                        ...
                        parameters_json: {
                            type: 'object',
                            description:
                                'The JSON schema for the tool parameters, potentially generalized beyond the original sample input.',
                        },
 ...
 * @param inject_agent_id The agent ID to inject as first parameter
...
---
TOOLS - You have access to the following helper functions available via 'tools':
${getHelperDescriptions().join('\n')}

---
SIGNATURE - The function signature you create MUST be:
\`\`\`typescript
   async function ${toolSpec.name}(agent_id${paramKeys.length > 0 ? ', ' + paramKeys.join(', ') : ''}) {
     // implementation
     const file = tools.read_file('path/to/file.txt'); // Example usage of a helper function
...
You MUST write your complete function implementation to a file named '${toolSpec.name}.ts' in the current working directory.
After writing to this file, make sure the VERY LAST LINE of your output to standard out is ONLY the relative path to this file: './${toolSpec.name}.ts'

---
SCRIPTS & TESTING:
If you generate TypeScript scripts for testing or execution that need access to the Magi helper functions:
1. Execute them using: 'magi-run-tool <AGENT_ID> <PATH_TO_YOUR_SCRIPT> '<JSON_ENCODED_ARGS>''
2. Replace <AGENT_ID> with '${inject_agent_id}'
3. Replace <JSON_ENCODED_ARGS> with a valid JSON string containing all arguments
4. In your script, access tools via tools, agent ID via agentId

Example command to test your tool:
magi-run-tool ${inject_agent_id} ./${toolSpec.name}.ts '{"param1":"value1","param2":123}'
...
3. Save the implementation to '${toolSpec.name}.ts'
4. TEST, TEST, TEST using \`magi-run-tool ./${toolSpec.name}.ts\` - fix any issues and resolve the underlying problem, never add mock code or placeholders.
5. Make the VERY LAST LINE of your output the string 'complete'`;
...
--- TEST RESULT ---
This is the feedback from the last test of your implementation:
${feedback}

Please fix the implementation based on this feedback. If you need to re-rewrite the entire function, please do so.`;
        }
...
ERROR: <clear explanation of why the task cannot be completed>`;
        ...
        try {
            ...
            const absolutePath = path.resolve(process.cwd(), `./${toolSpec.name}.ts`);
            ...
            // First param should be agent_id, rest should match our schema
            if (funcParams.length === 0) {
                throw new Error('Function signature has no parameters');
            }

            // Verify that all required parameters are present in the function signature
            // Skip agent_id (first parameter) in the comparison
            const missingParams = paramKeys.filter(
                key => !funcParams.slice(1).includes(key)
            );
            ...
            executionResult = await safelyExecuteFunction(
                implementation,
                toolSpec.name,
                args,
                inject_agent_id // Pass the agent_id to provide context to the tool
            );

            console.log(`Tool execution result: ${executionResult}`);

            // Use quickLlmCall with reasoning_mini to verify the result
            const verifierResult = await quickLlmCall(
                {
                    name: 'VerifierAgent',
                    description:
...
Return only "pass" if it satisfies expectations, or "fail: <reason>" explaining why it doesn't.`,
                    modelClass: 'reasoning_mini',
                    modelSettings: {
...
ACTUAL RESULT: ${executionResult}`
            ...
            if (verifierResult.trim().toLowerCase().startsWith('pass')) {
                ...
                feedback =
                    'The tool produced output that does not match expectations. ' +
                    failReason;
...
/**
 * Safely executes a function string in a VM context with timeout
 * @param implementation The function implementation as a string
 * @param functionName The name of the function to call
 * @param args Arguments to pass to the function
 * @returns Promise that resolves with the function result
 */
async function safelyExecuteFunction(
    implementation: string,
    functionName: string,
    args: any[],
    agent_id?: string
): Promise<string> {
    // Use the shared tool executor to run the implementation
    console.log(
        `[safelyExecuteFunction] Executing tool function '${functionName}' using shared executor...`
    );
    return executeToolInSandbox({
 ...
 * Handles mapping between original input keys and snake_case parameter names
 * @param implementation Function implementation string
 * @param inject_agent_id Agent ID to inject as first parameter
 * @param input Input string or JSON object
 * @param parametersSchema Schema containing mapping between original and snake_case keys
...
function prepareArguments(
    ...
    const paramNames = extractParameterNames(implementation);

    if (paramNames.length === 0) {
        return []; // No parameters, return empty array
    }

    // First param is always inject_agent_id
    const args: any[] = [inject_agent_id];

    // If there are more params, try to map from JSON input
    if (paramNames.length > 1) {
        try {
            ...
            // Create reverse mapping from snake_case to original
            const reverseMapping: Record<string, string> = {};
            for (const [original, snakeCase] of Object.entries(keyMapping)) {
                reverseMapping[snakeCase as string] = original;
            }

            // If input is an object, map properties to parameters by name
            if (typeof inputObj === 'object' && inputObj !== null) {
                // Skip first parameter (inject_agent_id) and map rest
                for (let i = 1; i < paramNames.length; i++) {
                    const paramName = paramNames[i];

                    // Try to find the parameter in the input object
                    // 1. Direct match with param name
                    if (paramName in inputObj) {
                        args.push(inputObj[paramName]);
                    }
                    // 2. Using the reverse mapping (snake_case -> original)
                    else if (
                    ...
                    // 3. Nothing found, use undefined
                    else {
                        args.push(undefined);
                    }
                }
            } else {
                // JSON input is not an object (e.g., array, primitive)
                // Just append it as second parameter
                args.push(inputObj);
            }
        } catch {
            // Not valid JSON, use as string parameter
            if (paramNames.length > 1) {
                args.push(input);
            }
        }
...
This tool is now available in your toolset.`;
        ...
        const output = await safelyExecuteFunction(
            generatedTool.implementation,
            generatedTool.name,
            args,
            inject_agent_id
    ...
    try {
        ...
        functionImpl = async (...args: any[]): Promise<string> => {
            try {
                ...
                return await safelyExecuteFunction(
                    tool.implementation,
                    tool.name,
                    args,
                    agent_id
