...
export async function acquireSlot(messageId: string): Promise<ClaudeSlot> {
  ...
  try {
    ...
    const countResult = await client.query('SELECT COUNT(*) FROM claude_slots');
    ...
    console.log(`[ClaudeDBLimiter] Acquired slot ${slotId} (${currentCount + 1}/${MAX_SLOTS} active)`);
      ...
      try {
        ...
        try {
          await heartbeatClient.query(`
            ...
            WHERE slot_id = $1
          `, [slotId]);
          console.log(`[ClaudeDBLimiter] Heartbeat for slot ${slotId}`);
        } finally {
          heartbeatClient.release();
        }
      } catch (error) {
        console.error(`[ClaudeDBLimiter] Error updating heartbeat for ${slotId}:`, error);
      }
...
export async function releaseSlot(slot: ClaudeSlot): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      await client.query('DELETE FROM claude_slots WHERE slot_id = $1', [slot.id]);
      console.log(`[ClaudeDBLimiter] Released slot ${slot.id}`);
    } finally {
      client.release();
    }
  } catch (error) {
    console.error(`[ClaudeDBLimiter] Error releasing slot ${slot.id}:`, error);
  }
}
