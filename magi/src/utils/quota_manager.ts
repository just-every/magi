...
import { QuotaUpdateEvent } from '../types.js';
...
export class QuotaManager {
    ...
    trackUsage(provider: ModelProviderID, model: string, inputTokens: number, outputTokens: number): boolean {
        ...
        if (quota.dailyLimit > 0) {
            const previousPercent = Math.floor((previousDailyUsed / quota.dailyLimit) * 10); // Percentage in tenths
            const currentPercent = Math.floor((quota.dailyUsed / quota.dailyLimit) * 10);
        ...
        if (provider === 'google') {
            ...
            if (model === 'gemini-2.5-pro-exp-03-25') {
                ...
                if (quota.dailyUsed >= quota.dailyLimit) {
                    console.log(`[QuotaManager] Gemini experimental model daily limit reached: ${quota.dailyUsed} > ${quota.dailyLimit}`);
            ...
            if (freeQuota.gpt4Models.includes(model)) {
                ...
                console.log(`[QuotaManager] OpenAI GPT-4 family usage: ${freeQuota.gpt4Family.used}/${freeQuota.gpt4Family.limit}`);
            ...
            if (freeQuota.gptMiniModels.includes(model)) {
                ...
                console.log(`[QuotaManager] OpenAI GPT-Mini family usage: ${freeQuota.gptMiniFamily.used}/${freeQuota.gptMiniFamily.limit}`);
    ...
    trackCreditUsage(provider: ModelProviderID, creditAmount: number): void {
        if (!this.quotas[provider] || this.quotas[provider].creditBalance === undefined) {
            return;
        }
        
        const quota = this.quotas[provider];
        if (quota.creditBalance !== undefined) {
            quota.creditBalance = Math.max(0, quota.creditBalance - creditAmount);
        }
    }
