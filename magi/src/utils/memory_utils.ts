...
function ensureMemoryDirectories(): void {
	try {
		...
		if (!fs.existsSync(path.dirname(SHORT_TERM_MEMORY_FILE))) {
			fs.mkdirSync(path.dirname(SHORT_TERM_MEMORY_FILE), { recursive: true });
		}
		if (!fs.existsSync(path.dirname(LONG_TERM_MEMORY_FILE))) {
			fs.mkdirSync(path.dirname(LONG_TERM_MEMORY_FILE), { recursive: true });
		}
	} catch (error) {
...
function loadMemoriesFromFiles(term?: string): void {
	try {
		...
		if ((!term || term === 'short') && fs.existsSync(SHORT_TERM_MEMORY_FILE)) {
			const shortData = fs.readFileSync(SHORT_TERM_MEMORY_FILE, 'utf-8');
			shortTermMemories = JSON.parse(shortData) || [];
			console.log(`Loaded ${shortTermMemories.length} short-term memories`);
		}

		// Load long-term memories
		if ((!term || term === 'long') && fs.existsSync(LONG_TERM_MEMORY_FILE)) {
...
function saveMemoriesToFiles(term: string): void {
	try {
		if(term === 'short') {
			...
			fs.writeFileSync(SHORT_TERM_MEMORY_FILE, JSON.stringify(shortTermMemories, null, 2), 'utf-8');
		}
		else if(term === 'long') {
			// Save long-term memories
			fs.writeFileSync(LONG_TERM_MEMORY_FILE, JSON.stringify(longTermMemories, null, 2), 'utf-8');
		}
...
function save_memory(term: string, memory: string): string {
	...
	try {
		...
		let memoryId:number;
		if (term === 'short') {
			memoryId = shortTermMemories.length > 0 ? (shortTermMemories[shortTermMemories.length - 1].id + 1) : 1;
			...
			memoryId = longTermMemories.length > 0 ? (longTermMemories[longTermMemories.length - 1].id + 1) : 1;
...
function find_memory(query: string[]): string {
	...
	try {
		...
		// Format the results
		const results = matchingMemories.map(item => {
...
function delete_memory(term: string, memoryId: number): string {
	...
	try {
		...
		if (term === 'short') {
			const shortTermIndex = shortTermMemories.findIndex(item => item.id === memoryId);
			if (shortTermIndex !== -1) {
			...
			const longTermIndex = longTermMemories.findIndex(item => item.id === memoryId);
			if (longTermIndex !== -1) {
...
export function listShortTermMemories(): string {
	return shortTermMemories.length > 0
		? shortTermMemories.map(memory => `- [${memory.id}] ${memory.content}`).join('\n')
		: '- None';
}
	...
	return [
		...
		createToolFunction(
			...
			'Find information in your long term memory.',
			{
				'query': {
					type: 'array',
					description: 'A list of terms to search your long term memory for. The search will return all memories that match any of the terms.',
				}
			},
