...
import { ModelProvider, StreamingEvent, ResponseInput, MessageEvent } from '../types.js';
	...
	async* createResponseStream(
       ...
       const yieldBufferedDelta = () => {
          ...
          if (deltaBuffer.length > 0) {
             console.log(`[ClaudeCodeProvider] Yielding buffered delta (${deltaBuffer.length} chars) for message ${messageId}`);
             eventQueue.push({
                type: 'message_delta',
                content: deltaBuffer,
       ...
       const startBatchTimer = () => {
           ...
           if (applicableTimeout === null) {
                console.warn('[ClaudeCodeProvider] No applicable batch timeout found, using fallback.');
                // Use the timeout from the last threshold (chars: 0) or a default
                applicableTimeout = batchTimeoutThresholds[batchTimeoutThresholds.length - 1]?.timeout ?? 4000;
           }
           ...
           console.log(`[ClaudeCodeProvider] Setting batch timer for ${applicableTimeout}ms (buffer: ${deltaBuffer.length} chars) for message ${messageId}`);
           currentBatchTimeoutValue = applicableTimeout; // Store the timeout value being set
           batchTimerId = setTimeout(() => {
               console.log(`[ClaudeCodeProvider] Batch timer (${applicableTimeout}ms) expired, yielding buffer for message ${messageId}.`);
       ...
       // --- End Helper Functions ---
		...
		try {
			...
			console.log(`[ClaudeCodeProvider] Starting stream for message ${messageId}`);
			yield {
				type: 'message_start',
			...
			const prompt = messages.map(msg => {
				let textContent = '';
				if ('content' in msg) { // Check if 'content' property exists
					if (typeof msg.content === 'string') {
						...
						textContent = msg.content
							...
							.map(part => (part as { type: 'input_text'; text: string }).text) // Type assertion
							.join('\n');
					}
				}
			...
			console.log(`[ClaudeCodeProvider] Executing streaming Claude CLI for model '${model}' in dir '${cwd}'...`);
			log_llm_request('anthropic', model, { // Assumes this function exists
				prompt: prompt.substring(0, 100) + (prompt.length > 100 ? '...' : ''), // Log truncated prompt
			...
			const completionPromise = new Promise<void>((resolve, reject) => {
				...
				const resetSilenceTimeout = () => {
					...
					silenceTimeoutId = setTimeout(() => {
						...
						console.error(`[ClaudeCodeProvider] PTY process timed out after ${silenceTimeoutDuration}ms of silence for message ${messageId}.`);
						ptyError = new Error(`Claude CLI process timed out after ${silenceTimeoutDuration/1000} seconds of silence.`);
						ptyExited = true; // Mark as exited due to timeout
						if (ptyProcess && typeof ptyProcess.kill === 'function') {
							try {
								console.log('[ClaudeCodeProvider] Attempting to kill PTY process due to silence timeout...');
								ptyProcess.kill(); // Attempt to kill the stalled process
							} catch(e) {
								console.warn('[ClaudeCodeProvider] Error killing timed out process', e);
							}
						}
				...
				try {
					...
					console.log(`[ClaudeCodeProvider] Spawning PTY: ${command} ${args.map(a => a.length > 50 ? a.substring(0, 50) + '...' : a).join(' ')}`);
					...
					ptyProcess.onData((data: string) => {
							...
							for (const line of lines) {
								...
								if (!processingStarted) {
									...
									if (!isNoiseLine(trimmedLine)) {
										if (isProcessingStartSignal(trimmedLine)) {
											processingStarted = true;
											console.log(`[ClaudeCodeProvider] Processing started detected for message ${messageId}.`);
								...
								if (!isNoiseLine(trimmedLine)) {
									...
									if (shouldBuffer) {
										...
										if (recentHistory.length > historySize) {
											const oldestLine = recentHistory.shift()!; // Remove oldest
                      ...
                      console.error('[ClaudeCodeProvider] Error processing PTY data:', processingError);
                      ptyError = new Error(`Error processing PTY data: ${processingError.message}`);
                      ptyExited = true; // Mark as exited due to error
                      if (silenceTimeoutId) clearTimeout(silenceTimeoutId);
                      if (batchTimerId) { // Clear batch timer on error
                          clearTimeout(batchTimerId);
                          batchTimerId = null;
                          currentBatchTimeoutValue = null;
                      }
                      if (ptyProcess && typeof ptyProcess.kill === 'function') {
                         try { ptyProcess.kill(); } catch(e) { console.warn('[ClaudeCodeProvider] Error killing process on data error', e); }
                      }
					...
					ptyProcess.onExit(({ exitCode, signal }) => {
						console.log(`[ClaudeCodeProvider] PTY process exited with code ${exitCode}${signal ? ` (signal ${signal})` : ''} for message ${messageId}.`);
						...
						try {
							...
							// WARNING: Relies on isNoiseLine
							if (processingStarted && finalTrimmedLine && !isNoiseLine(finalTrimmedLine)) {
								let shouldBufferFinal = true;
								if (finalTrimmedLine === lastYieldedLine) shouldBufferFinal = false;
								if (shouldBufferFinal && recentHistorySet.has(finalTrimmedLine)) shouldBufferFinal = false;
							...
							console.error('[ClaudeCodeProvider] Error processing final PTY buffer:', finalProcessingError);
						...
						try {
							...
							const costMatch = accumulatedCleanOutput.match(/Total cost\s*:\s*\$([\d.]+)/m);
							...
							console.warn(`[ClaudeCodeProvider] Failed to parse metadata from accumulated output: ${metadataParseError.message}`);
						}
						// --- End Metadata Extraction ---


						// Resolve or reject the completion promise based on outcome
						if (ptyError) { // If an error (like timeout) occurred before exit
                ...
                console.error('[ClaudeCodeProvider] Error spawning PTY process:', spawnError);
                ptyError = spawnError; // Set error flag
                ptyExited = true; // Mark as exited
                if (silenceTimeoutId) clearTimeout(silenceTimeoutId);
                if (batchTimerId) { // Clear batch timer on spawn error
                    clearTimeout(batchTimerId);
                    batchTimerId = null;
                    currentBatchTimeoutValue = null;
                }
			...
			if (ptyError && !String(ptyError).includes('timed out')) {
				...
				if (finalApiDuration !== null) metadata.apiDuration = finalApiDuration;
				if (finalWallDuration !== null) metadata.wallDuration = finalWallDuration;
				...
				// Log final status
				if (ptyError) { // Log timeout occurrence if relevant
					console.log(`[ClaudeCodeProvider] Stream finished after silence timeout for message ${messageId}.`);
				} else {
					console.log(`[ClaudeCodeProvider] Stream completed successfully for message ${messageId}.`);
				}
			}

		} catch (error: unknown) { // Catch any synchronous errors or re-thrown errors
			// 7. Handle errors (spawn, non-zero exit, processing, etc.)
			console.error('[ClaudeCodeProvider] Error during Claude Code streaming execution:', error);
          ...
          if (silenceTimeoutId) clearTimeout(silenceTimeoutId);
          if (batchTimerId) { // Ensure batch timer is cleared
              clearTimeout(batchTimerId);
              batchTimerId = null;
              currentBatchTimeoutValue = null;
          }

			console.log(`[ClaudeCodeProvider] Finishing stream processing for message ${messageId}.`);
