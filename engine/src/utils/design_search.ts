...
/**
 * Parse web search results into DesignSearchResult format
 */
function parseWebSearchResults(
    ...
    try {
        // Try to extract URLs and titles from the search result
        const results: DesignSearchResult[] = [];
        ...
        try {
            const jsonData = JSON.parse(searchResult);
            if (Array.isArray(jsonData)) {
                return jsonData
                    .filter(item => item && item.url)
                    .filter(item => {
                        // Exclude common non-design results
                        return (
        ...
        // Parse from text - look for URLs with context
        ...
        for (const line of lines) {
            ...
            if (urlMatch && urlMatch[0]) {
                ...
                // Use the line text as title, or the previous line if this line is just a URL
                let title = trimmedLine.replace(url, '').trim();
                if (!title && currentTitle) {
                    title = currentTitle;
                }
                if (!title) {
                    title = `${query} Design Example`;
                }
                ...
                // Save as potential title for next URL
...
export async function design_search(
    ...
    switch (engine) {
        ...
        default:
            ...
            const engines = ['anthropic', 'openai', 'google', 'sonar'];
            let searchResult = '';
